    ...
    for idx, node in enumerate(nodes):
        ...
        if dist_3d(new_node['coord'], node['coord']) < radius:
            ...
            if potential_cost < node['cost'] and no_collision(new_node['coord'], node['coord'], obstacles):
                node['parent'] = new_idx      # Update Parent the index of new_node
                node['cost'] = potential_cost # Update Cost
...
def calc_path(path, avgvel,total_time):
    ...
    for t in np.arange(0, time_taken , 0.05):       
    ...
    with open("/home/anuj/Desktop/AerialRobotics/apairaikar_p2a/final_traj.csv", mode="w", newline="") as file:
...
time_taken = calculate_time_taken(obs_coll_pru_path)
calc_path(obs_coll_pru_path, 2, time_taken)

x = [point[0] for point in obs_coll_pru_path]
y = [point[1] for point in obs_coll_pru_path]
z = [point[2] for point in obs_coll_pru_path]
...
# Optionally, to connect the points with lines:
