def smooth_path(path, max_iterations, no_collision):
    """
    path: list of points in the path (each point is assumed to be an array-like object)
    max_iterations: maximum number of smoothing attempts
    no_collision: a function that takes two points and returns True if there is no obstacle between them
    ...
    for _ in range(max_iterations):
        ...
        # Ensure the points are not adjacent and there is a path to smooth
        if idx2 - idx1 <= 1:
            continue
        
        # Check if the line segment b…
import numpy as np
...
map_file = "/home/anuj/Desktop/AerialRobotics/apairaikar_p2a/src/sample_maps/map1.txt"
...
def douglas_peucker(points, epsilon):
    ...
    # If max distance is greater than epsilon, recursively simplify both subpaths
    if dmax >= epsilon:
        recursive1 = douglas_peucker(points[:index + 1], epsilon)
        recursive2 = douglas_peucker(points[index:], epsilon)

        pruned_path = recursive1[:-1] + recursive2
    else:
        pruned_path = [points[0], points[end]]
...
def calc_path(path, avgvel, total_time):
    ...
    y_d_ddot = []
    z_d_ddot = []
    time_array = np.arange(0, total_time ,0.05)
    print("this is time array:",time_array , "and this is the tota time:", total_time)

    with open("/home/anuj/Desktop/AerialRobotics/apairaikar_p2a/sample_traj.csv", mode="w", newline="") as file:
        ...
        for t in time_array:
            ...
            print("this i :",i,"this is x0:",x0, y0, z0,"this is xf:",xf, yf, zf)

            
            if i == 0:
                tf = math.ceil(dist_3d(path[i+1], path[i]) / avgvel)
            
            print("this is time", t, "this is tf",tf)
            ...
            writer.writerow([x_d[-1], y_d[-1], z_d[-1], x_d_dot[-1], y_d_dot[-1], z_d_dot[-1], x_d_ddot[-1], y_d_ddot[-1], z_d_ddot[-1]])
...
sample_path = [np.array([1,0,0]), np.array([0,1,0]),np.array([0,0,2]),np.array([4,0,0])]


def calculate_time_taken(path, avgvel):
    tf = 0 
...
epsilon = 1
pruned_path = douglas_peucker(path, epsilon)
obs_coll_pru_path = prune_path(pruned_path, obstacles)
time_taken = calculate_time_taken(obs_coll_pru_path, 2)
calc_path(obs_coll_pru_path, 2, time_taken)

time_taken = calculate_time_taken(path, 2)
