...
sys.path.append('./src')
sys.path.append('/home/anuj/Desktop/AerialRobotics/YourDirectoryID_p0/Phase2/src/') #For custom python files import using absolute path
sys.path.append('/home/anuj/AerialRobotics/lib/python3.10/site-packages')
...
def main():
    ...
    while current_time < sim_stop_time:
        ...
        if user_countdown<=0.:
            ...
            xyz_bl_des = user_sm.step(current_time, xyz_blender)
            WP_ned = np.array([xyz_bl_des[0], -xyz_bl_des[1], -xyz_bl_des[2], 0.0])
            user_countdown = user_dt

        if control_countdown<=0.:
            #print('waypoint is ', WP_ned)
            U = controller.step(current_ned_state, WP_ned)
