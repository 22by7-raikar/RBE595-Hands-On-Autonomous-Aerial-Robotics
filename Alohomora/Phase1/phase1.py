...
for i in range((len(ts)-1)):
    roll = roll + gd[i+1][0]*(ts[i+1]-ts[i])
    pitch = pitch + gd[i+1][1]*(ts[i+1]-ts[i])
    yaw = yaw + gd[i+1][2]*(ts[i+1]-ts[i])
    rpy = [roll,pitch,yaw]
    gyro_rpy.append(rpy)
...
cf_orientation = []
for i in range(len(ts)-1): 
    roll = (1-alpha)*gyro_rpy[i][0] + alpha*acc_rpy[i][0]
    pitch = (1-alpha)*gyro_rpy[i][1] + alpha*acc_rpy[i][1]
    yaw = (1-alpha)*gyro_rpy[i][2] + alpha*acc_rpy[i][2] 
    orientation = rot_matrix(roll,pitch,yaw)
    cf_orientation.append(orientation)    
